...
for (let opt of categorySelect.options) {
  ...
  if (parts.some(part => optText === part)) {
...
if (!found) {
  for (let opt of categorySelect.options) {
    const optText = opt.textContent.trim().toLowerCase();
    if (parts.some(part => optText.includes(part))) {
      categorySelect.value = opt.value;
      found = true;
      break;
    }
...
// 🔹 3. If still not found, clear selection
if (!found) {
  categorySelect.value = '';
}

console.log('Matched?', found, 'Selected value:', categorySelect.value);
