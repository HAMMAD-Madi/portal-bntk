...
async function sendImage(blob) {
  ...
  try {
    ...
    if (!hits.length) {
      result.textContent = `No matches for â€œ${selectC.value}â€.`;
      return;
    }
...
// Render results & attach modal logic
function renderResults(items) {
  ...
  items.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <a class="link" href="${item.external_sites?.[0]?.url || '#'}" target="_blank">â†’ View on Bricklink</a>
    `;
