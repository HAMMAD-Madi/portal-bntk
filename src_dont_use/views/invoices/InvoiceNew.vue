    ...
    <v-card class="card z-index-2 h-100">
      ...
      >
        <v-icon size="x-large">fa-address-book</v-icon>
                  ...
                  <v-btn class="mt-3 text-white" color="#66bb6a" type="submit">Opslaan</v-btn>
                  <v-btn @click="debug">Foutopsporing</v-btn>
              ...
              <v-sheet style="border: 1px solid #ccc" class="mt-10 invoice-table">
                ...
                <div id="invoiceitems">
                  <div class="flex-container" v-for="(item, index) in fields" :key="index">
                    ...
                    <Field style="align-self: stretch;" v-bind="item.quantity" :name="`items[${index}].quantity`"
                      type="number" @input="recalculate" />
                    <Field style="align-self: stretch;" v-bind="item.margin" :name="`items[${index}].margin`"
                      type="number" step="any" @input="recalculate" />
                    <Field style="align-self: stretch;" v-bind="item.taxRate" :name="`items[${index}].taxRate`"
                      type="number" @input="recalculate" />
                    <Field style="align-self: stretch;" v-bind="item.price" :name="`items[${index}].price`"
                      type="number" step="any" @input="recalculate" />
                ...
                <v-btn class="ml-2 my-5 text-white" color="#2196f3"
                  @click="push({ description: '', quantity: 0, discount: 0, taxRate: 21, price: 0, order: 1 })">Nieuwe
                  regel</v-btn>
...
const updateContact = async () => {
  ...
  const fullname = { full_name: response.data.firstname || '' + ' ' + response.data.affix || '' + ' ' + response.data.lastname || '', value: response.data.id }
...
const onSubmit = handleSubmit(async (values) => {
  ...
  if (response.data.status == 'success') {
    if (response.data.invoice_number) {
      router.push('/facturen/' + response.data.invoice_number)

    } else {
      alert('Er is een fout opgetreden tijdens het opslaan van de factuur');
    }
  } else {
    alert('Er is een fout opgetreden tijdens het opslaan van de factuur');
  }
...
const downnloadPDF = async () => {
  ...
  const fileName = "factuur_" + invoiceNumber.value + "_" + contactSelected.value.lastname + ".pdf";
